/***************************************************************/
/* Code Writer (written by Anish M Mathew )                                 */
/*                                                                          */
/* This file was automatically generated using Code Writer                  */
/* Any manual changes to this file will be overwritten by a automated tool. */
/*                                                                          */
/* Date Generated : 16-03-2022 13:05:30                                      */
/*                                                                          */
/* More Details    --                                                       */
/*http://visualstudiogallery.msdn.microsoft.com/40d92d45-107e-4f83-b6c5-50a7e2419389*/
/****************************************************************************/
using System;
using System.Data;
using System.Linq;
using System.Text;
using System.Collections.Generic;
using Dapper;
using System.Threading.Tasks;
using MMS.data.Infrastructure;
using MMS.data.Entities;

namespace MMS.data.Repository
{
	public partial class RepresentativeRepository : BaseRepository, IRepresentativeRepository
	{
		public RepresentativeRepository(IConnectionFactory connectionFactory) : base(connectionFactory) { }
		
		#region Select One
		/// <summary>
		/// Get data according to the primary key value.
		/// </summary>
		/// <param name="id">System.Int32?</param>
		public async Task<Representative> Get(System.Int32? id)
		{
			using (var connection = connectionFactory.GetConnection)
			{
				var query = "[dbo].Representative_SELECT";
				var param = new DynamicParameters();
				param.Add(@"id", id, DbType.Int32);
				var list = await SqlMapper.QueryAsync<Representative>(connection, query, param, commandType: CommandType.StoredProcedure);
				return list.FirstOrDefault();
				
			}
		}
		#endregion
		
		#region Search
		public async Task<IEnumerable<Representative>> Search(int pageIndex, int pageSize)
		{
			
			using (var connection = connectionFactory.GetConnection)
			{
				var query = "[dbo].Representative_PAGING";
				var param = new DynamicParameters();
				param.Add("@PageIndex", pageIndex);
				param.Add("@PageSize", pageSize);
				
				var list = await SqlMapper.QueryAsync<Representative>(connection, query, param, commandType: CommandType.StoredProcedure);
				
				if (list == null)
				return null;
				else
				return list;
				
			}
		}
		public async Task<IEnumerable<Representative>> Search(int pageIndex, int pageSize,string sortBy, string orderBy)
		{
			
			using (var connection = connectionFactory.GetConnection)
			{
				var query = "[dbo].Representative_SORT";
				var param = new DynamicParameters();
				param.Add("@PageIndex", pageIndex);
				param.Add("@PageSize", pageSize);
				param.Add("@sortBy", sortBy);
				param.Add("@orderBy", orderBy);
				
				var list = await SqlMapper.QueryAsync<Representative>(connection, query, param, commandType: CommandType.StoredProcedure);
				
				if (list == null)
				return null;
				else
				return list;
				
			}
		}
		public async Task<IEnumerable<Representative>> Search(int pageIndex, int pageSize,string sortBy, string orderBy, string searchstring)
		{
			
			using (var connection = connectionFactory.GetConnection)
			{
				var query = "[dbo].Representative_CUSTOM";
				var param = new DynamicParameters();
				param.Add("@PageIndex", pageIndex);
				param.Add("@PageSize", pageSize);
				param.Add("@sortBy", sortBy);
				param.Add("@orderBy", orderBy);
				param.Add("@searchstring", searchstring);
				
				var list = await SqlMapper.QueryAsync<Representative>(connection, query, param, commandType: CommandType.StoredProcedure);
				
				if (list == null)
				return null;
				else
				return list;
				
			}
		}
		public async Task<IEnumerable<Representative>> Search(System.Int32? id, System.String accountId, System.String name, System.String email, System.String officePhone, System.String cellPhone, System.String imageURL, System.String notes)
		{
			try
			{
				
				using (var connection = connectionFactory.GetConnection)
				{
					var query = "[dbo].Representative_SEARCH";
					var param = new DynamicParameters();
					param.Add("id",id, DbType.Int32);
					param.Add("accountId",accountId, DbType.String);
					param.Add("name",name, DbType.String);
					param.Add("email",email, DbType.String);
					param.Add("officePhone",officePhone, DbType.String);
					param.Add("cellPhone",cellPhone, DbType.String);
					param.Add("imageURL",imageURL, DbType.String);
					param.Add("notes",notes, DbType.String);
					
					var list = await SqlMapper.QueryAsync<Representative>(connection, query, param, commandType: CommandType.StoredProcedure);
					
					if (list == null)
					return null;
					else
					return list;
					
				}
			}
			catch (Exception ex)
			{
				throw ex;
			}
		}
		#endregion
		
		#region INSERT
		/// <summary>
		/// Insert current Representative to database.
		/// </summary>
		/// <param name="accountId">System.String</param>
		/// <param name="name">System.String</param>
		/// <param name="email">System.String</param>
		/// <param name="officePhone">System.String</param>
		/// <param name="cellPhone">System.String</param>
		/// <param name="imageURL">System.String</param>
		/// <param name="notes">System.String</param>
		public async Task<System.Int32> Insert(System.String accountId, System.String name, System.String email, System.String officePhone, System.String cellPhone, System.String imageURL, System.String notes)
		{
			try
			{
				
				using (var connection = connectionFactory.GetConnection)
				{
					var query = "[dbo].Representative_INSERT";
					var param = new DynamicParameters();
					param.Add("accountId", accountId, DbType.String);
					param.Add("name", name, DbType.String);
					param.Add("email", email, DbType.String);
					param.Add("officePhone", officePhone, DbType.String);
					param.Add("cellPhone", cellPhone, DbType.String);
					param.Add("imageURL", imageURL, DbType.String);
					param.Add("notes", notes, DbType.String);
					var objs = await SqlMapper.ExecuteScalarAsync<System.Int32>(connection, query, param, commandType: CommandType.StoredProcedure);
					return objs;
					
				}
			}
			catch (Exception ex)
			{
				throw ex;
			}
		}
		/// <summary>
		/// Insert current Representative to database.
		/// </summary>
		/// <param name=Representative Objects>Representative</param>
		public async Task<System.Int32> Insert(Representative model)
		{
			try
			{
				
				using (var connection = connectionFactory.GetConnection)
				{
					var query = "[dbo].Representative_INSERT";
					var param = new DynamicParameters();
					param.Add("accountId", model.AccountId, DbType.String);
					param.Add("name", model.Name, DbType.String);
					param.Add("email", model.Email, DbType.String);
					param.Add("officePhone", model.OfficePhone, DbType.String);
					param.Add("cellPhone", model.CellPhone, DbType.String);
					param.Add("imageURL", model.Image_URL, DbType.String);
					param.Add("notes", model.Notes, DbType.String);
					var objs = await SqlMapper.ExecuteScalarAsync<System.Int32>(connection, query, param, commandType: CommandType.StoredProcedure);
					return objs;
					
				}
			}
			catch (Exception ex)
			{
				throw ex;
			}
		}
		#endregion
		
		#region UPDATE
		/// <summary>
		/// Update current Representative to database.
		/// </summary>
		/// <param name="id">System.Int32?</param>
		/// <param name="accountId">System.String</param>
		/// <param name="name">System.String</param>
		/// <param name="email">System.String</param>
		/// <param name="officePhone">System.String</param>
		/// <param name="cellPhone">System.String</param>
		/// <param name="imageURL">System.String</param>
		/// <param name="notes">System.String</param>
		public async Task<int> Update(System.Int32? id, System.String accountId, System.String name, System.String email, System.String officePhone, System.String cellPhone, System.String imageURL, System.String notes)
		{
			try
			{
				
				using (var connection = connectionFactory.GetConnection)
				{
					var query = "[dbo].Representative_UPDATE";
					var param = new DynamicParameters();
					param.Add("id", id, DbType.Int32);
					param.Add("accountId", accountId, DbType.String);
					param.Add("name", name, DbType.String);
					param.Add("email", email, DbType.String);
					param.Add("officePhone", officePhone, DbType.String);
					param.Add("cellPhone", cellPhone, DbType.String);
					param.Add("imageURL", imageURL, DbType.String);
					param.Add("notes", notes, DbType.String);
					var objs = await SqlMapper.ExecuteAsync(connection, query, param, commandType: CommandType.StoredProcedure);
					
					return objs;
					
				}
			}
			catch (Exception ex)
			{
				throw ex;
			}
		}
		public async Task<int> Update(Representative model)
		{
			try
			{
				
				using (var connection = connectionFactory.GetConnection)
				{
					var query = "[dbo].Representative_UPDATE";
					var param = new DynamicParameters();
					param.Add("id", model.Id, DbType.Int32);
					param.Add("accountId", model.AccountId, DbType.String);
					param.Add("name", model.Name, DbType.String);
					param.Add("email", model.Email, DbType.String);
					param.Add("officePhone", model.OfficePhone, DbType.String);
					param.Add("cellPhone", model.CellPhone, DbType.String);
					param.Add("imageURL", model.Image_URL, DbType.String);
					param.Add("notes", model.Notes, DbType.String);
					var objs = await SqlMapper.ExecuteAsync(connection, query, param, commandType: CommandType.StoredProcedure);
					return objs;
					
				}
			}
			catch (Exception ex)
			{
				throw ex;
			}
		}
		#endregion
		
		#region DELETE
		/// <summary>
		/// Delete current Representative from database.
		/// </summary>
		/// <param name="id">System.Int32?</param>
		public async Task<int> Delete(System.Int32? id)
		{
			using (var connection = connectionFactory.GetConnection)
			{
				var query = "[dbo].Representative_DELETE";
				var param = new DynamicParameters();
				param.Add("id", id, DbType.Int32);
				var iValue = await SqlMapper.ExecuteAsync(connection, query, param, commandType: CommandType.StoredProcedure);
				return iValue;
				
			}
		}
		#endregion
		
		
	}
}

